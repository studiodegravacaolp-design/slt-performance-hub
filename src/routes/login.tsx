      ...
      <aside className="relative hidden flex-col justify-between overflow-hidden bg-gradient-brand p-12 lg:flex">
        ...
        <div className="space-y-6 text-primary-foreground">
          ...
          <p className="max-w-md text-base text-primary-foreground/80">
            Gerencie empresas, filiais, atletas e treinos em uma plataforma multi-tenant segura.
          </p>
          <div className="grid gap-3 pt-4">
            <Feature
              icon={<ShieldCheck className="h-4 w-4" />}
              text="Isolamento por tenant via RLS"
            />
            <Feature
              icon={<Sparkles className="h-4 w-4" />}
              text="Insights de performance com IA"
            />
            <Feature
              icon={<Activity className="h-4 w-4" />}
              text="Periodização e prescrição inteligente"
            />
          </div>
        </div>
        <p className="text-xs text-primary-foreground/60">
          © {new Date().getFullYear()} System Level Tech
        </p>
      </aside>
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <p className="text-center text-xs text-muted-foreground">
              Novos acessos são provisionados pelo gestor responsável.
            </p>
          </form>
        </div>
      </main>
...
function Feature({ icon, text }: { icon: React.ReactNode; text: string }) {
  return (
    <div className="flex items-center gap-3 text-sm text-primary-foreground/90">
      <div className="grid h-7 w-7 place-items-center rounded-md bg-primary-foreground/10">
        {icon}
      </div>
      {text}
    </div>
